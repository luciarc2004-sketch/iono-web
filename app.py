...
st.set_page_config(page_title="Monitoreo Ionosférico TEC", layout="wide")
...
def generar_enlace_dlr_europa_actual():
    # Buscamos datos recientes aproximados (2 horas atrás para asegurar disponibilidad en el repo histórico)
    ahora = datetime.datetime.utcnow() - datetime.timedelta(hours=2)
    anio, mes, dia, hora = ahora.year, ahora.month, ahora.day, ahora.hour
    minuto = (ahora.minute // 15) * 15 # Redondeo a intervalos del DLR
...
def geocodificar_localidad(nombre_lugar):
    """Obtiene latitud y longitud usando la API pública OpenStreetMap Nominatim"""
    try:
...
# Carga y procesado de matrices
@st.cache_data(ttl=900) # Caché de 15 minutos para agilizar la web
...
try:
    ...
    # Definición de mallas para interpolación
    ...
    # =====================================================================
    # SECCIÓN DE CONSULTA DE LOCALIDAD
    # =====================================================================
    ...
    map_eur = ax1.pcolormesh(grid_lon_eur, grid_lat_eur, matriz_vtec_eur, transform=ccrs.PlateCarree(),
                             cmap='jet', alpha=0.85, shading='gouraud', zorder=2)
    ...
    map_glb = ax2.pcolormesh(grid_lon_glb, grid_lat_glb, matriz_vtec_glb, transform=ccrs.PlateCarree(),
    ...
    st.error(f"Error al conectar con los servidores del DLR o procesar los archivos JSON: {e}")
